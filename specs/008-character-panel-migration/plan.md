# Implementation Plan: CharacterPanel 이관

**Branch**: `008-character-panel-migration` | **Date**: 2026-04-15 | **Spec**: [spec.md](./spec.md)
**Input**: Feature specification from `/specs/008-character-panel-migration/spec.md`

## Summary

레거시 Electron 대시보드의 `CharacterPanel`을 `migration` Tauri + React 앱으로 동일하게 포팅한다.
이번 범위는 데이터 연결이나 동작 추가 없이, 패널 카드 외형과 내부 정사각형 비주얼 영역을 레거시와 같은 형태로 재현하는 것이다.
...
### 1. 레거시 존중 ✅ PASS
- 레거시 기준 소스는 `src/renderer/src/features/dashboard/ui/CharacterPanel.tsx`로 고정한다.
- `src/`는 읽기 전용으로 분석만 수행하고, 수정은 계획 범위에 포함하지 않는다.
- 구현 및 검증 문서에서 원본 파일 경로를 명시해 정합성을 추적한다.

### 2. UI 충실도 보존 ✅ PASS
- 카드 외곽선, 흰 배경, 둥근 모서리, 내부 정사각형 영역과 배경 톤을 동일하게 유지한다.
...
- 후속 패널이 참조할 수 있는 “레거시 동일 복제” 기준 사례를 먼저 만든다.
...
**Phase 1 재검토 결과**: ✅ ALL PASS
- 설계 산출물에서도 레거시 원본 유지, UI 스타일 불변, 무의존성 이관 원칙이 유지된다.
- 신규 저장소, API, 외부 패키지, 시스템 명령이 설계에 포함되지 않았다.
...
```text
specs/008-character-panel-migration/
├── plan.md
├── spec.md
├── research.md
├── data-model.md
├── quickstart.md
├── contracts/
│   └── character-panel-ui-contract.md
└── tasks.md
```
...
│   ├── main-panels/
│   │   ├── ui/
│   │   │   └── CharacterPanel.tsx          # 신규 정적 패널 컴포넌트
│   │   └── model/
...
└── features/
    └── dashboard/
        └── ui/
            └── CharacterPanel.tsx          # 레거시 기준 구현
```

**Structure Decision**: 기존 마이그레이션 앱의 FSD 구조를 유지한다. CharacterPanel은 데이터 의존성이 없는 정적 패널이므로 `migration/src/features/main-panels/ui/`에 단일 컴포넌트로 추가하고, 공통 타입과 스타일 토큰은 기존 모듈을 재사용한다. 대시보드 실제 배치 연결은 레거시 사용처 확인 결과를 기준으로 최소 범위에서만 수행한다.
